...
3. **Counter-D1-CRT trade gate** (#4 above) — behavioral change, needs explicit backtest evidence. **BENCH SHIPPED 2026-04-26 (Followup D), gate DEFERRED.** `scripts/bench_counter_d1_crt.py` walks the closed ICT_Bridge trades in `~/.tradingview-mcp/trading_ledger.db`, replays D1 CRT detection at each entry timestamp via cached M15 resampled to D1 (matches the bridge's `df_d1[:-1]` convention), and buckets trades into aligned/counter/no_crt. First run on 2026-04-26 (frozen artifact `scripts/bench_counter_d1_crt_2026-04-26.txt`): 36 closed trades total, 26 classified after dropping symbols with no M15 cache (DOGEUSD/UKOIL/YM1!/DAX/USDJPY/EURJPY/GBPJPY/GER40.CASH = 10 trades skipped). Buckets — aligned: N=16, WR 43.8%, mean R +0.08; counter: N=10, WR 30.0%, mean R -0.08; no_crt: N=0 (every classified trade had an active D1 CRT). WR delta (all - counter) = +8.5pp, below the +10pp ship threshold AND counter-N=10 is below the N≥30 power threshold. Both gates fail → defer. Re-run after every ~10 new live trades; the threshold is N(counter)≥30 AND WR delta ≥10pp. Note: counter total P&L is misleadingly *positive* (+$956) due to one $1,872 XAUUSD outlier — exclude-the-outlier reasoning is exactly the contamination the N≥30 rule is designed to avoid.

---

## Completed 2026-04-26 (Counter-D1-CRT bench — Followup D, gate deferred)

One commit shipped the evidence-gathering harness without touching live behavior:

- **`d60521f` — feat(bench): counter-D1-CRT trade-gate evidence harness.** Adds `scripts/bench_counter_d1_crt.py` + frozen first-run output `scripts/bench_counter_d1_crt_2026-04-26.txt`. Bench reads closed ICT_Bridge trades from `~/.tradingview-mcp/trading_ledger.db`, slices each symbol's M15 cache to bars strictly before `entry_time`, resamples to D1, calls `detect_crt(df_d1[:-1], lookback=1, tf_label="D1")` (matching the bridge's call site), and tags trades as aligned/counter/no_crt. Decision rule: SHIP a `-5` penalty gate iff N(counter) ≥ 30 AND WR(counter) ≤ WR(all) - 10pp. First run: aligned 16 trades (43.8% WR), counter 10 trades (30.0% WR), WR delta +8.5pp. Both thresholds fail → DEFER. No code path in `synergy_scorer.py` was changed — this commit is pure evidence-gathering, no behavioral change.

Net effect: the gate stays unbuilt, but the evidence-gathering machinery is now in place. Future sessions can re-run the bench after every ~10 new live trades; once N(counter)≥30 and WR delta meets threshold, the gate ships in a separate commit. The frozen artifact prevents the "phrase over-iteration" failure mode (running the bench, getting an answer we don't like, tweaking thresholds until we do).

### Findings surfaced during Followup D (NOT yet acted on)

1. **Cache coverage gap** — 10 of 36 closed trades skipped because trading-ai-v2 M15 cache only has 7 symbols (BTC/ETH/EUR/GBP/SOL/US500/XAU). DOGEUSD/UKOIL/YM1!/DAX/USDJPY/EURJPY/GBPJPY/GER40.CASH have no cache so the replay-D1 step can't run. Either expand the cache or fetch from MT5/TradingView at bench time. Tracking as a future task; doesn't block the gate decision (the 26-trade subset is the binding constraint).
2. **Cache currency** — the cache only covers through 2026-04-20; trades after that date have insufficient post-entry history but the bench only needs PRE-entry history so this is OK. If the cache stalls, future bench runs will quietly miss new trades — worth a freshness check.
3. **No detect_crt sample-rate normalisation** — the bench resamples the full M15 cache to D1 each time; for 17K bars × 26 trades that's ~half a second. If trades grow into the hundreds, cache the resample once per symbol.
4. **Outlier sensitivity** — one XAUUSD counter-CRT trade returned +$1,872 (R=+2.45). It's a real entry, not contaminated, but reading P&L instead of WR makes the bucket look favourable. The bench correctly uses WR + mean R as the decision signal; just worth flagging to anyone re-reading the artifact.
...
1. ~~**`get_session_range` is incomplete for broad sessions**~~ — **RESOLVED 2026-04-26 (Followup B leftover #1).** Added `get_broad_session_range(df, hour_start, hour_end, n_recent_sessions=1) -> list[(high, low)]` to `Desktop/trading-ai-v2/analysis/sessions.py`. Pure NY-hour interval math, no `_classify_session` dependency — handles same-day windows (e.g. 02–10 NY for broad London) and wrap-around windows that straddle midnight (e.g. 20–02 NY, attributes to next NY trading date to match the Asian-session convention used in `detect_session_crt`). Validates inputs (rejects out-of-range hours, zero-width windows, empty/non-DatetimeIndex frames). Four unit tests in `tests/unit/test_session_range.py` cover same-day, two-recent-sessions, midnight wrap, and invalid-input cases. `detect_session_crt` itself was NOT refactored — its trading-date attribution shifts the Asian range forward by one day to pair with London, which is detector-specific bookkeeping that doesn't generalize to a pure-helper API. The new helper is the path future detectors should use; `detect_session_crt`'s in-place hour math is left as documented prior art. Files committed in trading-ai-v2 as `07973b1` (helper + 4 unit tests, 213 insertions); no bridge-side wiring because no bridge code calls the helper yet — it's infrastructure for future broad-window detectors.
