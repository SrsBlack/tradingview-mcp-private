...
- **`<pending hash>` — feat(bench): counter-D1-CRT trade-gate evidence harness.** Adds `scripts/bench_counter_d1_crt.py` + frozen first-run output `scripts/bench_counter_d1_crt_2026-04-26.txt`. Bench reads closed ICT_Bridge trades from `~/.tradingview-mcp/trading_ledger.db`, slices each symbol's M15 cache to bars strictly before `entry_time`, resamples to D1, calls `detect_crt(df_d1[:-1], lookback=1, tf_label="D1")` (matching the bridge's call site), and tags trades as aligned/counter/no_crt. Decision rule: SHIP a `-5` penalty gate iff N(counter) ≥ 30 AND WR(counter) ≤ WR(all) - 10pp. First run: aligned 16 trades (43.8% WR), counter 10 trades (30.0% WR), WR delta +8.5pp. Both thresholds fail → DEFER. No code path in `synergy_scorer.py` was changed — this commit is pure evidence-gathering, no behavioral change.
